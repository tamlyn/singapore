  ...
  switch($sg->action) {
    case "addgallery" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"add")) {
        ...
        $adminMessage = $sg->translator->_g("Gallery added");
        $includeFile = "editgallery";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
    ...
    case "addimage" :
      ...
      switch($_REQUEST["sgLocationChoice"]) {
        ...
        case "single" :
          if($sg->addImage()) {
            $adminMessage = $sg->translator->_g("Image added");
        ...
        case "multi" :
          if($sg->addMultipleImages()) {
            $adminMessage = $sg->translator->_g("Archive contents added");
            $includeFile = "view";
          } else {
            $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
            $includeFile = "newimage";
    ...
    case "changethumbnail" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"edit")) {
        ...
        if($sg->saveGalleryThumbnail())
          $adminMessage = $sg->translator->_g("Thumbnail changed");
        else
          $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "editgallery";
      } elseif(isset($_REQUEST["confirmed"]) && $_REQUEST["confirmed"]==$sg->translator->_g("confirm|Cancel")) {
        $includeFile = "editgallery";
      } else {
    ...
    case "deletegallery" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"delete")) {
        $adminMessage = $sg->translator->_g("You do not have permission to perform this operation.");
        $includeFile = "view";
      } elseif(isset($_REQUEST["confirmed"]) && $_REQUEST["confirmed"]==$sg->translator->_g("confirm|OK") || (count($sg->gallery->images)==0 && count($sg->gallery->galleries)==0)) {
        if($sg->deleteGallery()) {
          $sg->selectGallery($sg->ancestors[0]->id);
          $adminMessage = $sg->translator->_g("Gallery deleted");
        } else {
          $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        }
        $includeFile = "view";
    ...
    case "deleteimage" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"delete")) {
        ...
        if($sg->deleteImage())
          $adminMessage = $sg->translator->_g("Image deleted");
        else
          $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "view";
      } elseif(isset($_REQUEST["confirmed"]) && $_REQUEST["confirmed"]==$sg->translator->_g("confirm|Cancel")) {
        ...
        $confirmTitle = $sg->translator->_g("delete image");
        $confirmMessage = $sg->translator->_g("Are you sure you want to irretrievably delete image %s from gallery %s?","<em>".$sg->imageName().$sg->imageByArtist()."</em>","<em>".$sg->gallery->name."</em>");
        $includeFile = "confirm";
      }
      break;
    case "deleteuser" :
      if(!$sg->user->isAdmin()) {
        ...
        if($sg->deleteUser())
          $adminMessage = $sg->translator->_g("User deleted");
        else
          $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        ...
        $confirmTitle = $sg->translator->_g("delete user");
        $confirmMessage = $sg->translator->_g("Are you sure you want to permanently delete user %s?","<em>".$_REQUEST["user"]."</em>");
        $includeFile = "confirm";
      }
      break;
    case "editgallery" :
    ...
    case "login" :
      if($sg->doLogin()) {
        $adminMessage = $sg->translator->_g("Welcome to singapore admin!");
        $includeFile = "menu";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "login";
    ...
    case "logout" :
      $sg->logout();
      $adminMessage = $sg->translator->_g("Thank you and goodbye!");
      $includeFile = "login";
      break;
    case "manageusers" :
      if(!$sg->user->isAdmin()) {
    ...
    case "purgecache" :
      if(!$sg->user->isAdmin()) {
        ...
        if($sg->purgeCache())
          $adminMessage = $sg->translator->_g("Thumbnail cache purged");
        else
          $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "menu";
      } elseif(isset($_REQUEST["confirmed"]) && $_REQUEST["confirmed"]==$sg->translator->_g("confirm|Cancel")) {
        ...
        $confirmTitle = $sg->translator->_g("purge cached thumbnails");
        $dir = $sg->getListing($sg->config->pathto_cache,"all");
        $confirmMessage = $sg->translator->_g("Are you sure you want to delete all %s cached thumbnails?",count($dir->files));
        $includeFile = "confirm";
      }
      break;
    case "reindex" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"edit"))
        ...
        $adminMessage = $sg->translator->_g("Gallery re-indexed. %s images added.",$imagesAdded);
      }
      $includeFile = "view";
      break;
    case "savegallery" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"edit")) {
        ...
        $adminMessage = $sg->translator->_g("Gallery info saved");
        $includeFile = "view";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "editgallery";
    ...
    case "saveimage" :
      ...
      if(!$sg->checkPermissions($sg->gallery,"edit")) {
        ...
        $adminMessage = $sg->translator->_g("Image info saved");
        $includeFile = "view";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "view";
    ...
    case "savepass" :
      if($sg->user->isGuest()) {
        ...
        $adminMessage = $sg->translator->_g("Password saved");
        $includeFile = "menu";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "editpass";
    ...
    case "savepermissions" :
      ...
      if(!$sg->user->isAdmin() && !$sg->user->isOwner($sg->gallery)) {
        ...
        $adminMessage = $sg->translator->_g("Permissions saved");
        $includeFile = "view";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "editpermissions";
    ...
    case "saveprofile" :
      if($_REQUEST["user"] != $sg->user->username || $sg->user->isGuest()) {
        ...
        $adminMessage = $sg->translator->_g("User info saved");
        $includeFile = "menu";
      } else {
        $adminMessage = $sg->translator->_g("An error occurred:")." ".$sg->getLastError();
        $includeFile = "editprofile";
